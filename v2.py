...
class VoteButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        cur.execute("SELECT is_active FROM polls WHERE id = ?", (self.poll_id,))
        poll = cur.fetchone()
        if not poll:
            conn.close()
        ...
        cur.execute(
            "SELECT option_text FROM poll_options WHERE id = ? AND poll_id = ?",
...
async def register_persistent_poll_views():
    ...
    for poll in polls:
        cur.execute("SELECT id, option_text FROM poll_options WHERE poll_id = ?", (poll["id"],))
...
async def parent_schedule_autocomplete(interaction: discord.Interaction, current: str):
    ...
    cur.execute("""
        ...
        WHERE s.guild_id = ? AND s.parent_id IS NULL
        ORDER BY s.id ASC
    """, (guild_id,))
    rows = cur.fetchall()
    conn.close()
...
async def vote_create(interaction: discord.Interaction, question: str, options: str):
    ...
    for opt in option_list:
        cur.execute("""
            ...
            VALUES (?, ?)
        """, (poll_id, opt))
        option_rows.append({"id": cur.lastrowid, "option_text": opt})
    ...
    cur.execute("UPDATE polls SET message_id = ? WHERE id = ?", (str(message.id), poll_id))
    conn.commit()
    conn.close()
...
async def food_delete(interaction: discord.Interaction, food_id: str):
    ...
    cur.execute("DELETE FROM food_votes WHERE food_id = ?", (food_id_int,))
    cur.execute("""
        DELETE FROM foods
        WHERE id = ? AND guild_id = ?
    """, (food_id_int, str(interaction.guild_id)))
...
async def schedule_add_sub(
    ...
    cur.execute("""
        ...
        FROM schedules
        WHERE id = ? AND guild_id = ? AND parent_id IS NULL
...
async def schedule_list(interaction: discord.Interaction, trip_id: str, day_number: int):
    ...
    cur.execute("""
        SELECT *
        FROM schedules
        WHERE guild_id = ? AND trip_id = ? AND day_number = ? AND parent_id IS NULL
        ORDER BY
